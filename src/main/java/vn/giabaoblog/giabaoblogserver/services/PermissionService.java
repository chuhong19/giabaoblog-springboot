...
import vn.giabaoblog.giabaoblogserver.config.exception.InvalidPermissionDataException;
import vn.giabaoblog.giabaoblogserver.config.exception.NotFoundException;
import vn.giabaoblog.giabaoblogserver.config.exception.PermissionInUseException;
...
public class PermissionService {
    ...
    public Permission getPermissionByKey(String key) {
        ...
        throw new NotFoundException(String.format("Permission not found for permission key = %s", key));
    }
    ...
    public Permission createPermission(PermissionDTO permissionDTO) {
        ...
        if (permissionOpt.isPresent()) {
            throw new NotFoundException(String.format("Permission %s already existing with the same key with Id = %s", permissionKey, permissionOpt.get().getId()));
        }
    ...
    public Permission updatePermission(PermissionDTO permissionDTO) {
        ...
        if (!permissionOpt.isPresent()) {
            throw new NotFoundException(String.format("The permission with the id = %s has not been found",
                    permissionId));
        }
    ...
    public void deletePermissionByKey(String permissionKey) {
        ...
        Optional<Permission> permissionOpt = permissionRepository.findByPermission(permissionKey);
        if (!permissionOpt.isPresent()) {
            throw new NotFoundException(String.format("Permission %s not found", permissionKey));
        }
